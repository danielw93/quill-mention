...
class Mention {
	...
	insertItem(data) {
		...
		const mentionString = data.denotationChar + data.value;

		this.quill.deleteText(this.mentionCharPos, this.cursorPos - this.mentionCharPos, Quill.sources.USER);
		console.log(data);
		this.quill.insertText(prevMentionCharPos, mentionString, 'mention', data, Quill.sources.USER);
		this.quill.setSelection(prevMentionCharPos + mentionString.length + 1, Quill.sources.USER);
